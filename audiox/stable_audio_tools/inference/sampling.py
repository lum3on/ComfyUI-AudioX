...
# Import k_diffusion with error handling
try:
    import k_diffusion as K
    # Verify that external module is available
    if not hasattr(K, 'external'):
        raise ImportError("k_diffusion.external not found")
    if not hasattr(K.external, 'VDenoiser'):
        raise ImportError("k_diffusion.external.VDenoiser not found")
except ImportError as e:
    print(f"k_diffusion import error: {e}")
    # Try alternative import
    try:
        import importlib
        K = importlib.import_module('k_diffusion')
        if not hasattr(K, 'external') or not hasattr(K.external, 'VDenoiser'):
            raise ImportError("k_diffusion.external.VDenoiser not available")
    except Exception as e2:
        raise ImportError(f"Could not import k_diffusion: {e}, {e2}")
...
def sample_k(
    ...
    try:
        ...
        raise RuntimeError(f"Failed to create K.external.VDenoiser: {e}. This often indicates an incompatible k-diffusion version. ComfyUI-AudioX expects a version like 0.0.14. Please check your installed k-diffusion version.")
