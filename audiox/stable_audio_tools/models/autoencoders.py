...
# Try to import DAC components with fallback
try:
    from dac.nn.layers import WNConv1d, WNConvTranspose1d
    DAC_AVAILABLE = True
except ImportError as e:
    print(f"Warning: DAC not available: {e}")
    print("Some AudioX features may not work. Install with: pip install descript-audio-codec")
    DAC_AVAILABLE = False
...
class DACEncoderWrapper(nn.Module):
    def __init__(self, in_channels=1, **kwargs):
        ...
        if not DAC_AVAILABLE:
            raise ImportError("DAC is required for DACEncoderWrapper. Install with: pip install descript-audio-codec")

        try:
            from dac.model.dac import Encoder as DACEncoder
        except ImportError:
            raise ImportError("DAC model components not available. Install with: pip install descript-audio-codec")
...
class DACDecoderWrapper(nn.Module):
    def __init__(self, latent_dim, out_channels=1, **kwargs):
        ...
        if not DAC_AVAILABLE:
            raise ImportError("DAC is required for DACDecoderWrapper. Install with: pip install descript-audio-codec")
